...
import { FaCode, FaReact, FaNodeJs, FaJava, FaPython, FaJsSquare, FaHtml5, FaCss3Alt, FaBootstrap, FaAws } from "react-icons/fa";
import { MdOutlineDataObject, MdArchitecture } from "react-icons/md";
import { SiTailwindcss, SiDocker, SiAmazoncloudwatch, SiKubernetes, SiAmazonec2, SiAwslambda, SiRedis, SiMongodb, SiPostman, SiPostgresql, SiJenkins, SiMysql, SiGithub, SiSpring, SiCypress, SiStorybook, SiAmazondynamodb, SiApachekafka, SiJunit5, SiGithubactions } from "react-icons/si";
...
const certIcons = {
  "Oracle Cloud Infrastructure Foundations Associate 2021": <GrOracle size={36} />,
};
...
const Skills = () => {
  ...
  return (
    <section className="mt-16 max-w-7xl mx-auto px-4 md:px-8" id="skills">
      <SectionHeader title={"Skills."} subtitle={"Empowering My Code with the Best Tools and Technologies"} />
      <div className="text-center md:text-left">
        <div className="overflow-hidden">
          <div className="flex flex-wrap justify-center gap-4 py-4">
            {skillsWork.slice(0, isMobile && !isExpandedSkills ? 15 : skillsWork.length).map((skill, idx) => (
              <div
                key={idx}
              ...
              >
                <div className="flex items-center gap-2">
                  {skillIcons[skill] || <FaCode size={14} className="text-secondary" />}
                  <span className="whitespace-normal font-semibold text-xs text-left">{skill}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-8">
        <SectionHeader title={"Certifications."} subtitle={"Unlocking New Heights: Recognitions That Shape My Journey"} />
        <motion.div
        ...
        >
          {certificates.slice(0, isMobile && !isExpandedCerts ? 3 : certificates.length).map((certificate, idx) => (
            <div
              key={idx}
              className="flex flex-col items-center gap-2 p-2 text-secondary text-center transition-all transform hover:scale-105 hover:border-secondary hover:bg-opacity-10 hover:text-white cursor-pointer rounded-md"
            >
              <div className="flex items-center justify-center gap-2">
                <div className="flex items-center justify-center gap-2">
                  {certIcons[certificate] || <GrOracle size={14} className="text-secondary" />}
                  <span className="whitespace-normal font-semibold text-xs text-center">{certificate}</span>
                </div>
              </div>
            </div>
          ))}
